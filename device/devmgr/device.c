...
ER tk_srea_dev(ID dd, W start, void *buf, SZ size, SZ *asize) {
    ...
    if (dev_tbl[dd].opncnt == 0) return E_ID;               // オープンされていない
    if ((dev_tbl[dd].omode & TD_READ) == 0) return E_OACV;  // オープンモードが読み込みでない
...
ER tk_swri_dev(ID dd, W start, const void *buf, SZ size, SZ *asize) {
    ...
    if (dd >= DEV_NUM) return E_ID;                         // ddが不正
    if (dev_tbl[dd].opncnt == 0) return E_ID;               // オープンされていない
    if ((dev_tbl[dd].omode & TD_WRITE) == 0) return E_OACV; // オープンモードが書き込みでない
