...
void task_btn(INT stacd, void *exinf) {
    ...
    // GP13をスイッチ入力に設定
    out_w(GPIO(13), (in_w(GPIO(13)) | GPIO_PUE) & ~GPIO_PDE);   // PULLUP設定
    out_w(GPIO_OE_CLR, (1<<13));                                // 出力無効
    out_w(GPIO_CTRL(13), 5);

    // GP14をスイッチ入力に設定
    out_w(GPIO(14), (in_w(GPIO(14)) | GPIO_PUE) & ~GPIO_PDE);   // PULLUP設定
    out_w(GPIO_OE_CLR, (1<<14));                                // 出力無効
    out_w(GPIO_CTRL(14), 5);
    
    btn0 = in_w(GPIO_IN) & ((1<<14)|(1<<13));
    while (1) {
        btn = in_w(GPIO_IN) & ((1<<14)|(1<<13));  // スイッチ読み込み
        if ((diff = btn ^ btn0) != 0) {
